...
def DepthLimit():
    n_qubits_range = np.arange(2, 9)
    depth_limit_ansatz1 = [2**(i-1) * (i-1) * (i-1) + (i * (i-1) / 2 + 1) * (i-1) for i in n_qubits_range]
    depth_limit_ansatz2 = [(2**(i) * (i) * (i-1) * i / 2) * (4 * i + 1) for i in n_qubits_range]
    ...
    line, = ax.semilogy(n_qubits_range, depth_limit_ansatz1, color='blue', lw=1, label = 'QAOA-E1')
    line2, = ax.semilogy(n_qubits_range, depth_limit_ansatz2, color='red', lw=1, label = 'QAOA-M1')
    plt.legend()
    plt.show()